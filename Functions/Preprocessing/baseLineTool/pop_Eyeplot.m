% pop_Eyeplot() - Visually inspect EEG data using a scrolling display.
...
% Usage:
%   >> pop_Eyeplot( EEG ) % Scroll EEG channel data. Allow marking for rejection via
...
%   >> pop_Eyeplot( EEG, icacomp, superpose, reject );
%
% Graphic interface:
...
function com = pop_Eyeplot( EEG, icacomp, superpose, reject, topcommand, varargin)
...
if nargin < 3 & EEG.trials > 1
    ...
    result = inputgui( { [ 2 0.2] [ 2 0.2]} , uilist, 'pophelp(''pop_Eyeplot'');', ...
                       fastif(icacomp==0, 'Manual component rejection -- pop_Eyeplot()', ...
								'Reject epochs by visual inspection -- pop_Eyeplot()'));
...
if EEG.nbchan > 100
    disp('pop_Eyeplot() note: Baseline subtraction disabled to speed up display');
...
com = [ com sprintf('pop_Eyeplot( %s, %d, %d, %d);', inputname(1), icacomp, superpose, reject) ]; 
return;
