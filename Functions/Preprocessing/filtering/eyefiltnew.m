% eyefiltnew() - Filter data using Hamming windowed sinc FIR filter
%
% Usage:
%   >> [EEG, com, b] = eyefiltnew(Eye); % pop-up window mode
%   >> [EEG, com, b] = eyefiltnew(Eye, locutoff, hicutoff, filtorder,
...
%
% Outputs:
%   Eye       - filtered Eye structure
%   com       - history string
%   b         - filter coefficients
%
% Note:
%   eyefiltnew is intended as a replacement for the deprecated
%   lowpass function. Required filter order/transition band width is
...
function [EYE, com, b] = eyefiltnew(EYE, locutoff, hicutoff, filtorder, revfilt, usefft, plotfreqz, minphase)
...
if isempty(filtorder)
    ...
    filtorder = 3.3 / (df / EYE.srate); % Hamming window
    filtorder = ceil(filtorder / 2) * 2; % Filter order must be even.
    
else

    df = 3.3 / filtorder * EYE.srate; % Hamming window
    filtorderMin = ceil(3.3 ./ ((maxDf * 2) / EYE.srate) / 2) * 2;
    filtorderOpt = ceil(3.3 ./ (maxDf / EYE.srate) / 2) * 2;
...
if plotfreqz
    freqz(b, 1, 8192, EYE.srate);
end

% Filter
if minphase
   % disp('pop_eegfiltnew() - filtering the data (causal)');
    EYE = firfiltsplit(EYE, b, 1);
else
